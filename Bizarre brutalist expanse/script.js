...
class Root {
  ...
  update() {
    ...
    if (this.size < this.maxSize) {
      ctx.beginPath();
      // ctx.rect(this.x, this.y, this.size, 0, Math.PI * 2);
      ctx.arc(this.x, this.y, this.size, 0, Math.PI * 2);
      ctx.fill();
      ctx.fillStyle = "hsl(0,0%," + this.lightness + "%)";
...
function isInBoundingBox(mouseX, mouseY, canvasBounds) {
  const isInHorizontal =
    mouseX >= canvasBounds.x && mouseX <= canvasBounds.width;
  const isInVertical =
    mouseY >= canvasBounds.y && mouseY <= canvasBounds.height;
